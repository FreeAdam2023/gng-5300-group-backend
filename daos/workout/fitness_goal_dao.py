...
class FitnessGoalDAO:
    def __init__(self):
        ...
        with self.db_client as db_client:
            ...
            db_client.ensure_validation(
                self.collection_name, "fitness_goals_schema.json"
            )
            db_client.db[self.collection_name].create_index(
                [("user_id", pymongo.ASCENDING)],
                unique=True,  # Ensure each user can only have one goal
            )
    ...
    def get_goal_by_user_id(self, user_id, db_client=None):
        ...
        if db_client is None:
            with self.db_client as db_client:
                goal = db_client.find_one(
                    self.collection_name, {"user_id": ObjectId(user_id)}
                )
        else:
            goal = db_client.find_one(
                self.collection_name, {"user_id": ObjectId(user_id)}
    ...
    def create_or_update_fitness_goal(
        self, user_id, goal, days_per_week, workout_duration, rest_days
    ):
