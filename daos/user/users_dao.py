...
class UserDAO:
    def __init__(self):
        ...
        with self.db_client as db_client:
            logger.info(
                f"Setting up validation and index for collection: {self.collection_name}"
            )
    ...
    def update_last_login(self, user_id):
        ...
        with self.db_client as db_client:
            result = db_client.update_one(
                self.collection_name, query, {"$set": update_data}
            )
    ...
    def set_password_reset_token(self, email):
        ...
        token = hashlib.sha256(
            f"{email}{datetime.utcnow().timestamp()}".encode()
        ).hexdigest()
    ...
    def update_user_info(self, user_id, update_fields):
        ...
        with self.db_client as db_client:
            result = db_client.update_one(
                self.collection_name, query, {"$set": update_fields}
            )
