...
async def get_workout_log(request: Request, log_date: str):
    ...
    logger.info(
        f"API: Fetching workout log for user_id {user_id} on log_date {log_date}"
    )
...
@handle_response
@auth_service.requires_auth
async def create_or_update_workout_log(
    request: Request, body: CreateOrUpdateWorkoutLogRequest
):
    """
    ...
    logger.info(
        f"API: Creating or updating workout log for user_id {user_id} on log_date {workout_log_date}"
    )
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Workout log successfully created/updated",
            "data": result,
        }
...
@handle_response
@auth_service.requires_auth
async def update_workout_log_fields(
    request: Request, body: UpdateWorkoutLogFieldsRequest, log_date: str
):
    """
    ...
    user_id = request.state.user_id  # Retrieve user_id from request.state
    logger.info(
        f"API: Updating workout log fields for user_id {user_id} on log_date {log_date}"
    )
    try:
        ...
        if result["matched_count"] == 0:
            raise HTTPException(
                status_code=404, detail="Workout log not found for update"
            )
        return {
            "status": "success",
