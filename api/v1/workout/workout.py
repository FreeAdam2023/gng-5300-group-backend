...
@handle_response
@auth_service.requires_auth
async def create_or_update_fitness_goal(
    request: Request, body: CreateOrUpdateGoalRequest
):
    """
    Create or update the user's fitness goal.
    """
    user_id = request.state.user_id  # Retrieve user_id from request.state
    result = service.create_or_update_fitness_goal(
        user_id=user_id, data=body.dict(exclude_unset=True)
    )
...
async def update_fitness_goal_fields(request: Request, body: CreateOrUpdateGoalRequest):
    ...
    try:
        ...
        serialized_result = {
            ...
            "upserted_id": (
                str(result["data"].get("upserted_id"))
                if result["data"].get("upserted_id")
                else None
            ),
        }
        ...
        if serialized_result["matched_count"] == 0:
            raise HTTPException(
                status_code=404, detail="Fitness goal not found for update"
            )
