...
def retrieve_answer(input_data):
    ...
    response_schemas = [
        ...
        ResponseSchema(
            name="Exercises",
            description="List of exercises included in the workout, with instructions for each exercise",
        ),
        ResponseSchema(
        ...
        ResponseSchema(
            name="Additional Tips", description="Any additional tips for the user"
        ),
        ResponseSchema(
    ...
    response = chain.invoke({"input_documents": matching_docs, "question": question})
